...
<div id="mod-rjcw_id<?php echo $moduleID; ?>" class="mod-rjcw">
	...
	<div class="mod-rjcw_c">
		...
		<div class="mod-rjcw_cleft">
			<img class="mod-rjcw-icon_big" src="<?php echo $this->icon($owm ? $now->id : $now->code, $params, $nighticon); ?>"
			height="96px"
			...
			<p class="mod-rjcw-current_temp">
				<?php echo $this->temp($current->temp, $params); ?>
			</p>
		</div>
		...
		<div class="mod-rjcw_cright">
			...
			<div class="mod-rjcw-row"><?php echo Text::_('MOD_RJCWEATHER_FEELS'); ?>: <?php echo $this->temp($owm ? $current->feels_like : $current->app_temp, $params); ?></div>
			<?php } ?>

			<?php if ($owm && $params->get('todayhi', 1)==1) { ?>
			<div class="mod-rjcw-row"><?php echo Text::_('MOD_RJCWEATHER_HITODAY'); ?>: <?php echo $this->temp($forecasts[0]->temp->max, $params); ?></div>
			<?php } ?>
		</div>
...
?>
			<div class="list_<?php echo ($i%2 ? 'even' : 'odd') ?>">
				<span class="mod-rjcw_list_day">
					<?php echo $this->dow($owm ? $fcast->dt : $fcast->ts); ?>
				</span>
				<span class="mod-rjcw_list_icon">
					<img class="mod-rjcw-icon" src="<?php echo $this->icon($wcc,$params); ?>"
					align="center"
					title="<?php echo $desc; ?>"
					alt="<?php echo $desc; ?>" />
					<?php if ($fcast->pop): ?>
				...
				<span class="mod-rjcw_list_temp">
					...
					echo $this->temp($htmp, $params) . '&nbsp;' . $params->get('separator', '/') . '&nbsp;' . $this->temp($ltmp, $params);
					?>
				</span>
				<div style="clear:both"></div>
			</div>
